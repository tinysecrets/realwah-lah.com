...
frontend/             React SPA (Vercel) - user interface
...
## Repository Layout (Live Topology)

```
wah-lah.com ──► Vercel (React SPA, frontend/)
    │
    └── /api/legal/*, etc. ──► api.wah-lah.com ──► Cloudflare Worker (wah-lah-api-proxy)
                                                        │
                                                        ▼
                                              Render (realwah-lah-com-8v2l.onrender.com)
                                                        │
                                                        ▼
                                              MongoDB Atlas
```

- `frontend/vercel.json` holds the SPA fallback + security headers for the Vercel deployment.
- `backend/render.yaml` (root `Dockerfile`) is the Render Blueprint for the API.
- The Cloudflare Worker source lives in `wah-lah-api-proxy/`. Deploy with `wrangler deploy` (or keep it in sync with the repo — the worker adds fallback 502 handling on backend outage).
