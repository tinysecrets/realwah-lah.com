...
- Persona webhook: signature-verified, upserts kyc_profiles.
...
def build_compliance_router(db, get_current_user, get_admin_user) -> APIRouter:
    ...
    async def kyc_upload(
        ...
        if tier not in (TIER_BASIC, TIER_ENHANCED):
            raise HTTPException(status_code=400, detail="tier must be 'basic' or 'enhanced'")
        if doc_type not in ("id_front", "id_back", "selfie", "proof_of_address"):
        ...
        # Sanitize the extension to alphanumerics only — the raw client
        # filename must never influence the storage path (traversal risk).
        raw_ext = (file.filename or "bin").rsplit(".", 1)[-1][:8].lower()
        ext = "".join(c for c in raw_ext if c.isalnum()) or "bin"
        safe_user = "".join(c for c in str(user["id"]) if c.isalnum() or c in ("-", "_"))
        path = KYC_UPLOAD_DIR / f"{safe_user}_{tier}_{doc_type}_{doc_id}.{ext}"
    ...
    async def admin_payout_action(payload: PayoutActionPayload, request: Request):
        ...
        # The btc_payouts_enabled kill switch is honored FIRST: with it off,
        # approvals record nothing and never touch the chain (the operator
        # then pays manually or re-enables and re-approves).
        if payload.action == "approve":
            from services.feature_flags import get_flags as _get_flags

            _flags = await _get_flags(db)
            if not _flags.get("btc_payouts_enabled", False):
                raise HTTPException(
                    status_code=503,
                    detail="BTC payouts are disabled (kill switch off). Decision not recorded.",
                )
            try:
