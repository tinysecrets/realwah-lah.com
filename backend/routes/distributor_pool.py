...
POST   /api/ext/pool/admin/health/rebalance trigger the credit rebalancer (admin/cron)
POST   /api/ext/pool/admin/transfer         manually trigger a P2P transfer (admin)
POST   /api/ext/pool/admin/proxies/{id}/test-pull   pinned pull-to-seat debug (admin)
POST   /api/ext/pool/admin/pull             manual pull via pull-capable pool (admin)
GET    /api/ext/pool/admin/pulls            pool_pull attempt history (admin)
"""
...
from services.pool_resync import resync_seat_balances
...
async def execute_pool_transfer(
    ...
    UNITS: ``amount`` is PLATFORM DOLLARS (22.00) — what the seat balance holds
    and what the hub form/API receives. ``amount_credits`` is the INTERNAL
    Game Credits figure (2,200), used only for the playthrough decrement.
    Callers must convert via ``credits_to_platform_amount`` before calling.
    ...
    if ok:
        ...
        # as these credits are now "in play" on the platform. Playthrough is tracked
        # in INTERNAL credits, so use amount_credits (fall back to amount for legacy
        # direct callers that still pass internal units).
        if user_id:
            try:
                ...
                await db.users.update_one(
                    ...
                    [{"$set": {
                        "playthrough_balance": {
                            "$max": [0, {"$subtract": ["$playthrough_balance",
                                                      amount_credits if amount_credits is not None else amount]}]
                        }
                    }}]
                )
            except Exception as e:
...
async def execute_pool_pull(
    ...
    """Reclaim Game Credits from a player's game account back to a seat.

    UNITS: ``amount`` is PLATFORM DOLLARS (what the hub pull moves);
    ``amount_credits`` is the INTERNAL Game Credits figure, kept for KPI
    continuity (defaults to ``amount`` for legacy direct callers).

    Mirror of ``execute_pool_transfer`` for the credit loop (pool_pull). Only
    pull-capable seats (see hub_registry ``pull_supported``) are eligible when
    no ``proxy_id`` is pinned. Every real attempt is written to the
    ``pool_pulls`` log and the result is classified in the returned detail as
    ``pull_status``: ``done`` | ``unsupported`` | ``failed`` so callers (e.g.
    the redemption loop) can stamp the same status on the source doc.
    ...
    if proxy_id:
        ...
        doc = await db[proxy_pool.COLLECTION].find_one({"_id": _OID(proxy_id)})
        if not doc:
            return False, "Proxy not found", {"proxy_id": None, "pull_status": "failed"}
        proxy = doc
    else:
        proxy, reason = await select_proxy(db, amount, platform=platform, require_pull=True)
        if not proxy:
            # No seat can pull yet — not a failure, just wholesale fallback.
            return False, reason, {"proxy_id": None, "pull_status": "skipped"}

    proxy_oid = proxy["_id"]
    creds = await get_decrypted_credentials(db, str(proxy_oid))
    if not creds:
        return False, "Proxy credentials unavailable", {
            "proxy_id": str(proxy_oid), "pull_status": "failed",
        }

    bridge = None
    diagnostic: Dict[str, object] = {}
    ok = False
    msg = "pull not attempted"
    try:
        from services.hub_bridge import make_bridge
        ...
        ok, msg, diagnostic = await bridge.pull(
            recipient=recipient_username, amount=amount, platform=platform,
        )
    except ImportError as e:
        logger.warning("Playwright unavailable: %s", e)
        ...
        logger.exception("Bridge pull crashed")
        await mark_failed(db, proxy_oid, f"crash: {e}")
        return False, f"Bridge exception: {e}", {
            "proxy_id": str(proxy_oid),
            "proxy_label": proxy.get("label"),
    ...
    # Classify: a hub that doesn't declare a pull endpoint is *unsupported*,
    # not a seat failure — the seat is fine, so we don't punish it.
    unsupported = (not ok) and ("not implemented" in msg.lower())
    status = "done" if ok else ("unsupported" if unsupported else "failed")

    clean_msg = msg
    if not ok and ("/app/" in msg or "/root/" in msg or "executable not found" in msg.lower()):
        clean_msg = "Bridge pull failed due to internal environment error. Check logs."

    if ok:
        # Pull restores balance — touch last_used_at but do NOT count as outbound volume.
        await mark_used(db, proxy_oid, 0)
    elif not unsupported:
        await mark_failed(db, proxy_oid, msg)

    log_id = await pool_pull.record_pull(db, {
        "ref_kind": ref_kind,
        "ref_id": ref_id,
        "user_id": user_id,
        "recipient_username": recipient_username,
        "platform": platform,
        "amount": amount_credits if amount_credits is not None else amount,
        ...
        "message": clean_msg,
        "diagnostic": diagnostic,
        "completed_at": pool_pull.utc_now().isoformat(),
    })
...
def build_distributor_pool_router(db, get_admin_user) -> APIRouter:
    ...
    async def get_pool_health(request: Request):
        ...
        return {**health, "capacity_alert": alert}

    @router.post("/admin/health/rebalance")
    async def post_rebalance(request: Request, dry_run: bool = True):
        """Run the credit rebalancer (rules a/b/c).

        Safe by default: ``dry_run=true`` only computes the plan. Pass
        ``?dry_run=false`` to actually move surplus credits — which still
        requires ``POOL_REBALANCE_ENABLED=true``. Meant to be hit by a
        scheduler/cron.
        """
        await get_admin_user(request)
        return await run_rebalance(db, dry_run=bool(dry_run))

    @router.post("/admin/resync")
    async def post_resync(request: Request):
        """Refresh every active/cooldown seat's real balance from its hub now.

        Same best-effort pass the pool resync worker runs on its interval,
        audited to ``pool_resyncs``. Handy for an operator to force a refresh
        right after updating hub_registry balance selectors/paths.
        """
        await get_admin_user(request)
        return await resync_seat_balances(db)

    @router.get("/admin/resyncs")
    async def get_resyncs(request: Request, limit: int = 10):
        """Recent resync pass audit rows (``pool_resyncs`` collection)."""
        await get_admin_user(request)
        limit = max(1, min(int(limit), 100))
        cursor = db["pool_resyncs"].find({}).sort("started_at", -1).limit(limit)
    ...
    async def add_proxy(body: ProxyCreate, request: Request):
        ...
        return await create_proxy(
            ...
            pull_supported=body.pull_supported,
    ...
    @router.post("/admin/proxies/{proxy_id}/test-pull")
    async def test_pull(proxy_id: str, body: TransferBody, request: Request):
        """Trigger a pull *pinned* to a specific proxy (admin debug flow).

        Safe to run even on a hub that doesn't support pull — the bridge
        returns a clean "unsupported" and nothing is changed on the seat.
        """
        await get_admin_user(request)
    ...
    async def manual_pull(body: TransferBody, request: Request):
        """Admin-initiated pull via the pool (only pull-capable seats eligible)."""
        await get_admin_user(request)
        ok, msg, detail = await execute_pool_pull(
    ...
    @router.get("/admin/pulls")
    async def pull_history(request: Request, limit: int = 50):
        """Recent pool_pull attempts (audit / recycling stats)."""
        await get_admin_user(request)
        if limit < 1:
            limit = 50
        if limit > 500:
            limit = 500
        items = await db["pool_pulls"].find({}, {"_id": 0}).sort("created_at", -1).to_list(limit)
        return {"total": len(items), "items": items}

    @router.get("/admin/pulls/kpi")
    async def pull_kpis(request: Request, window_hours: int = 24):
        """Recycled-credit KPI: share of redemption value self-financed by pool_pull."""
        await get_admin_user(request)
        if window_hours < 1:
            window_hours = 24
        if window_hours > 24 * 30:
            window_hours = 24 * 30
        return await pool_pull.pool_pull_kpis(db, window_hours)
    ...
    proxy_pool.execute_pool_pull = execute_pool_pull  # type: ignore[attr-defined]
    return router
