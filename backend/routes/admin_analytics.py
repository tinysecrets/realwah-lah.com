...
class TicketRespondBody(BaseModel):
...
def get_analytics_routes(db):
    ...
    async def get_support_tickets(request: Request, status: Optional[str] = None):
        ...
        tickets = await db.support_tickets.find(
            query,
            {"_id": 1, "user_email": 1, "user_name": 1, "subject": 1, "message": 1,
             "status": 1, "priority": 1, "created_at": 1, "responses": 1,
             "genie_reply": 1, "source": 1}
        ...
        return [
            {
                ...
                "genie_reply": t.get("genie_reply"),
                "source": t.get("source", "player"),
                "status": t["status"],
    ...
    async def respond_support_ticket(ticket_id: str, body: TicketRespondBody, request: Request):
        """Operator reply — the player sees it in the Concierge tab."""
