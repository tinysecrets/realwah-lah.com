...
def build_extensions_router(db, get_current_user, get_admin_user) -> APIRouter:
    ...
    async def forgot_password(data: PasswordResetRequest):
        ...
        # The reset token is exposed ONLY when the operator explicitly opts a
        # non-production environment into it. Default is production-safe:
        # an unset APP_ENV must NEVER leak tokens (previously it did, because
        # render.yaml never set APP_ENV at all).
