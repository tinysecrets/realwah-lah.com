...
Flow on approval:
  1. Use the SINGLE funding address at m/0'/0/0 (funds must be consolidated
     there; other indexes are never spent — see send_btc).
  2. Fetch that address's UTXOs from BlockCypher.
  3. Build + sign a transaction server-side with `bit`, paying the user and
     sending any change back to the wallet's change key (m/0'/1/0).
...
Treat it like cash: secrets manager / KMS in production, never logs, never git.
Per-tx ceiling via BTC_PAYOUT_MAX_SAT (0 = uncapped).
"""
...
# Per-transaction ceiling (satoshis). Unset/0 = no cap. Set BTC_PAYOUT_MAX_SAT
# in production so one bad approval or bug cannot drain the hot wallet.
try:
    MAX_PAYOUT_SAT = int(os.getenv("BTC_PAYOUT_MAX_SAT", "0"))
except ValueError:
    MAX_PAYOUT_SAT = 0


def is_plausible_btc_address(addr: str) -> bool:
    """Cheap syntactic check for P2PKH / P2SH / bech32 mainnet addresses."""
    if not addr or not isinstance(addr, str):
        return False
    a = addr.strip()
    if a.startswith("bc1"):
        return 14 <= len(a) <= 90 and all(c not in a for c in (" ", "\n", "\t"))
    if a.startswith(("1", "3")):
        return 26 <= len(a) <= 35 and all(c not in a for c in (" ", "\n", "\t"))
    return False
...
async def send_btc(
    ...
    if not is_plausible_btc_address(to_address):
        return False, "invalid_btc_address", None
    if MAX_PAYOUT_SAT > 0 and amount_sat > MAX_PAYOUT_SAT:
        logger.warning("Payout of %d sat exceeds BTC_PAYOUT_MAX_SAT=%d — refused.", amount_sat, MAX_PAYOUT_SAT)
        return False, f"exceeds_max_payout_{MAX_PAYOUT_SAT}", None

    # 1. Gather UTXOs from the SINGLE funding address (index 0).
    # Correctness note: the transaction is signed with the index-0 key only,
    # so inputs from any other index would produce an invalid (rejected)
    # transaction. Keep ALL hot-wallet funds consolidated at funding_address(0);
    # anything elsewhere is simply not spendable by this sender.
    try:
        funding_key = _signing_key(0, 0)
        all_unspents = await _fetch_unspents(funding_key.address, funding_key)
    except PayoutError as e:
        return False, f"utxo_lookup_error_{e}", None
    total_available = sum(u.amount for u in all_unspents)
