...
/api/redemption/quote for BTC). Hidden fees → chargebacks
...
async def get_rates(db) -> Dict[str, float]:
    ...
    rates = {
        ...
        "deposit_bonus_rate": float((doc or {}).get("deposit_bonus_rate", 0.0) or 0.0),
        "deposit_bonus_playthrough_multiplier": float(
            (doc or {}).get("deposit_bonus_playthrough_multiplier", 1.0) or 1.0
        ),
    }
...
async def set_rates(db, *, cashtag: Optional[float] = None,
                    ...
                    deposit_bonus_rate: Optional[float] = None,
                    deposit_bonus_playthrough_multiplier: Optional[float] = None,
    ...
    if deposit_bonus_rate is not None:
        update["deposit_bonus_rate"] = max(0.0, min(0.5, float(deposit_bonus_rate)))
    if deposit_bonus_playthrough_multiplier is not None:
        update["deposit_bonus_playthrough_multiplier"] = max(
            0.0, min(10.0, float(deposit_bonus_playthrough_multiplier))
        )
