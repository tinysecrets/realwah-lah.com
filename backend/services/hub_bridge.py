...
def parse_money(text: Optional[str]) -> Optional[float]:
    """Pull a positive number out of a balance label like '$1,234.56 CAD'.

    Returns ``None`` when nothing numeric is found, so callers can treat the
    read as failed rather than recording a bogus zero balance. A leading minus
    (e.g. overdraft display "-$5.00") also returns ``None``.
    """
    if not text:
        return None
    raw = str(text)
    ...
    try:
        ...
        return None
    if value < 0 or raw.strip().startswith("-"):
        return None
    return value
...
class GenericHubBridge:
    ...
    async def get_balance(self) -> Tuple[bool, Optional[float], Dict[str, Any]]:
        """Read the seat's current balance. Return ``(ok, balance, diagnostic)``.

        Mirrors ``ping``'s diagnostic style. POSITION OF ``ok`` IS DIFFERENT
        from ping/transfer/pull: the 2nd element is the numeric balance (or
        ``None``) because the pool resync worker just wants the money.

        ``ok=False`` with ``diagnostic["balance_supported"]=False`` means the
        hub has no ``balance`` selector configured in ``hub_registry`` — the
        caller must treat that as *unsupported*, NOT a seat failure (the cached
        balance stays "unknown", so the floor rule can't mis-disable the seat).
        """
        sel = self.hub["selectors"]
        bal_sels = sel.get("balance") or []
        self.diagnostic["balance_supported"] = bool(bal_sels)
        if not bal_sels:
        ...
        # Land on the dashboard where the balance usually renders.
        dash = self.hub.get("dashboard_path")
        if dash:
            try:
                await self.page.goto(
                    self.base_url.rstrip("/") + dash,
                ...
                self._step("balance_nav", url=self.page.url)
            except Exception as e:
                self._step("balance_nav_error", error=str(e))

        bal_sel, bal_el = await self._first_match(bal_sels)
        self._step("balance_find", matched=bal_sel)
        if not bal_el:
            return False, None, self.diagnostic

        text = ""
        try:
            text = (await bal_el.inner_text() or "").strip()
        except Exception:
            pass
        if not text:
            try:
                text = (await bal_el.text_content() or "").strip()
            except Exception:
                pass
        if not text:
            try:
                text = (await bal_el.get_attribute("data-balance") or "").strip()
            except Exception:
                pass
        self._step("balance_read", selector=bal_sel, text=text[:80])
    ...
    async def pull(
        ...
        Not implemented for Playwright-driven hubs yet. Pull requires a
        verified hub-side API (see ``hub_registry`` pull contract); once a hub
        ships an ``api_base_url`` + ``api_paths.pull`` it is handled by
        ``HttpHubBridge`` instead. Returning a clean "unsupported" keeps the
        redemption loop on the wholesale fallback instead of double-spending.
        """
        return False, (
            f"Pull not implemented for hub '{self.hub_type}' — no verified "
            "pull endpoint. Redemption fell back to wholesale."
        ), self.diagnostic
