...
- /api/checkout/create opens a BTC deposit (URL or graceful 4xx)
...
def test_payment_card_info_cashtag_exact():
    # The real payment tag lives ONLY in env — never in the repo. Provide the
    # expected value out-of-band when running this live-backend test.
    expected = os.environ.get("EXPECTED_CARD_PAYMENT_TAG", "")
    if not expected:
        pytest.skip("EXPECTED_CARD_PAYMENT_TAG not set (real tag is never committed)")
    r = requests.get(f"{BASE_URL}/api/payment/card-info", timeout=10)
    assert r.status_code == 200
    j = r.json()
    assert j.get("tag") == expected, "cashtag mismatch"
...
def test_checkout_create_does_not_500(user_session):
    ...
    #   200 + url → BTC deposit address issued (ideal)
    #   4xx with clear message (e.g., 409 JIT registration hold, 451 geoblock)
