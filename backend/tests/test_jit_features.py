...
2. Each new user gets a unique per-user game_password (no shared preset)
...
class TestUsernameFormat:
    ...
    def test_register_creates_game_password(self):
        """POST /api/auth/register creates user with a per-user game_password"""
        ...
        # Check game_password is present and NOT the shared Abc123 preset
        assert "game_password" in data, "game_password not in response"
        assert data["game_password"] != "Abc123", "game_password must not be the shared preset"
        assert len(data["game_password"]) >= 12, "game_password should be at least 12 chars"
        
        print(f"✅ New user has a unique game_password: {data['game_password']}")
...
class TestJITCheckoutIntegration:
    ...
    def test_checkout_calls_jit_registration(self):
        ...
        # Should NOT be 409 (JIT failure) - may be 4xx/5xx from the BTC path
