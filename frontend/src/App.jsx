...
import { Eye, EyeOff, Gamepad2, CreditCard, Users, BarChart3, Settings, LogOut, History, Shield, Wallet, Copy, Check, ChevronRight, Sparkles, Star, Download, ArrowDownToLine, ArrowUpFromLine, RefreshCw, DollarSign, Menu, X, Clock, Activity, Wand2, Rocket, Gift, Server, Trophy, Zap, MessageCircle } from "lucide-react";
...
const AdminDailyOps = lazy(() => import("./components/AdminDailyOps"));
...
const CompetitionPromo = lazy(() => import("./components/CompetitionPromo"));
const AdminCompetition = lazy(() => import("./components/AdminCompetition"));
...
const Dashboard = () => {
  ...
  const tabs = [
    ...
    { id: "promo", label: "Win $1M", icon: <Trophy size={20} /> },
  ...
  return (
    <div className="dashboard">
      ...
      <main className="dashboard-content">
        {activeTab === "games" && (
          <div>
            {/* Million Dollar Finale banner */}
            <button
              type="button"
              className="comp-banner"
              data-testid="comp-banner"
              onClick={() => setActiveTab("promo")}
            >
              <Trophy size={22} />
              <span className="comp-banner-text">
                <strong>WIN $1,000,000</strong>
                <span>1 entry per $1 spent · free daily entries, no purchase necessary</span>
              </span>
              <ChevronRight size={20} className="comp-banner-arrow" />
            </button>
        ...
        {activeTab === "promo" && (
          <Suspense fallback={<LoadingFallback />}>
            <CompetitionPromo user={user} refreshUser={refreshUser} />
          </Suspense>
        )}
          ...
          return (
            ...
            >
              {(() => {
                ...
                const Logo = logoForName(game.name);
                if (!Cover && !Logo) return null;
                return (
                  <div className={`pd-cover ${registered ? "pd-cover-ok" : ""}`}>
                    {Logo ? (
                      <>
                        <img className="pd-cover-logo" src={Logo} alt={game.name}
                          onError={(e) => { e.target.style.display = "none"; }} />
                        {Cover ? <div className="pd-cover-fallback"><Cover /></div> : null}
                      </>
                    ) : (
                      <Cover />
                    )}
...
// Transactions Tab
const TX_KIND_META = {
  btc_deposit: { label: "BTC deposit", Icon: ArrowDownToLine },
  manual_deposit: { label: "Cash deposit", Icon: ArrowDownToLine },
  purchase: { label: "Token purchase", Icon: CreditCard },
  redemption: { label: "BTC cash-out", Icon: ArrowUpFromLine },
  giftcard: { label: "Gift card", Icon: Gift },
  grant: { label: "Bonus grant", Icon: Sparkles },
  adjustment: { label: "Account adjustment", Icon: RefreshCw },
};

const txStatusTone = (status) => {
  const s = (status || "").toLowerCase();
  if (["completed", "approved", "fulfilled", "granted", "done"].includes(s)) return "ok";
  if (["pending", "hold_admin_review", "settling", "awaiting_send", "processing"].includes(s)) return "wait";
  return "bad";
};

const txAmountText = (tx) => {
  if (tx.kind === "grant") {
    const c = Number(tx.game_credits || 0).toLocaleString();
    return `+${c} credits`;
  }
  if (tx.kind === "adjustment") {
    const inc = tx.inc || {};
    const parts = Object.entries(inc).map(([k, v]) => `${v >= 0 ? "+" : ""}${Number(v).toLocaleString()} ${k.replace(/_/g, " ")}`);
    return parts.join(" · ") || (tx.note || "updated");
  }
  const usd = tx.amount_usd;
  return usd === null || usd === undefined ? "—" : `$${Number(usd).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
};

const txDetailText = (tx) => {
  if (tx.kind === "btc_deposit" || tx.kind === "manual_deposit") {
    const bits = [];
    if (tx.platform) bits.push(tx.platform);
    if (tx.tx_ref) bits.push(`ref ${tx.tx_ref}`);
    if (tx.fee_usd) bits.push(`fee $${Number(tx.fee_usd).toFixed(2)}`);
    return bits.join(" · ");
  }
  if (tx.kind === "giftcard") return tx.brand || "";
  if (tx.kind === "redemption") return `${Number(tx.game_credits || 0).toLocaleString()} credits`;
  if (tx.kind === "purchase" && tx.sugar_tokens) return `${Number(tx.sugar_tokens).toLocaleString()} tokens`;
  if (tx.kind === "adjustment") return tx.note || "";
  if (tx.kind === "grant") return tx.grant_type || "";
  return "";
};

const TX_FILTERS = [
  { id: "all", label: "All" },
  { id: "btc_deposit", label: "BTC in" },
  { id: "manual_deposit", label: "Cash in" },
  { id: "redemption", label: "Cash-outs" },
  { id: "giftcard", label: "Gift cards" },
  { id: "grant", label: "Bonuses" },
...
const TransactionsTab = () => {
  ...
  const fetchTransactions = useCallback(async (k) => {
    setIsLoading(true);
    setFailed(false);
    try {
      const { data } = await axios.get(`${API}/user/transactions`, { params: { kind: k, limit: 100 } });
  ...
  useEffect(() => { fetchTransactions(kind); }, [fetchTransactions, kind]);

  return (
    <div className="tab-content transactions-tab">
      <div className="section-header">
        <h2>Ledger</h2>
        <p>Every deposit, cash-out, bonus and adjustment on this account</p>
      </div>

      {summary && (
        <div className="ledger-summary" data-testid="ledger-summary">
          <div className="ledger-stat"><span className="ledger-stat-num">${(summary.deposited_usd || 0).toFixed(2)}</span><span className="ledger-stat-label">Deposited</span></div>
          <div className="ledger-stat"><span className="ledger-stat-num">${(summary.received_usd || 0).toFixed(2)}</span><span className="ledger-stat-label">Received</span></div>
          <div className="ledger-stat"><span className="ledger-stat-num">{summary.pending_items || 0}</span><span className="ledger-stat-label">Pending</span></div>
        </div>
      )}

      <div className="ledger-filters" data-testid="ledger-filters">
        {TX_FILTERS.map((f) => (
          <button key={f.id} type="button" data-testid={`ledger-filter-${f.id}`}
            className={`chip-btn ${kind === f.id ? "active" : ""}`} onClick={() => setKind(f.id)}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      {isLoading ? (
        <div className="empty-state"><History size={48} /><p>Loading your ledger…</p></div>
      ) : failed ? (
        <div className="empty-state" data-testid="ledger-error">
          <History size={48} />
          <p>Couldn't load your ledger. Pull to retry.</p>
          <button type="button" className="btn-primary" onClick={() => fetchTransactions(kind)}>Retry</button>
        </div>
        ...
        <div className="transactions-list">
          {transactions.map((tx) => {
            const meta = TX_KIND_META[tx.kind] || { label: tx.kind, Icon: History };
            const Icon = meta.Icon;
            return (
              <div key={`${tx.kind}-${tx.id}`} className="transaction-row" data-testid="ledger-row">
                <div className="tx-icon"><Icon size={20} /></div>
                <div className="tx-details">
                  <span className="tx-game">{meta.label}</span>
                  {txDetailText(tx) && <span className="tx-sub">{txDetailText(tx)}</span>}
                  <span className="tx-date">{tx.created_at ? new Date(tx.created_at).toLocaleString() : ""}</span>
                </div>
                <div className="tx-side">
                  <span className="tx-amount">{txAmountText(tx)}</span>
                  {tx.status && <span className={`badge tx-status-${txStatusTone(tx.status)}`}>{String(tx.status).replace(/_/g, " ")}</span>}
                </div>
              </div>
            );
          })}
...
// Concierge lives in src/components/Concierge.jsx (Genie chat + ticket threads).
...
const AdminPanel = () => {
  ...
  const fetchData = useCallback(async () => {
    const [statsRes, usersRes, gamesRes, txRes] = await Promise.allSettled([
      axios.get(`${API}/admin/stats`),
      axios.get(`${API}/admin/users`, { params: { limit: 100 } }),
      axios.get(`${API}/games/all`),
      axios.get(`${API}/admin/transactions`, { params: { limit: 100 } })
    ]);
    let failed = 0;
    ...
    if (failed > 0) toast.error(`${failed} admin feed${failed > 1 ? "s" : ""} failed to load`);
  }, []);
  ...
  const handleAdjustBalance = async (adjustData) => {
    try {
      await axios.patch(`${API}/admin/users/${selectedUser.id}`, adjustData);
      toast.success("Balance adjusted (audited)");
      fetchData();
      setShowPaymentModal(false);
      setSelectedUser(null);
    } catch (err) { toast.error(err.response?.data?.detail || "Failed to adjust balance"); }
  };

  const handleUpdateUser = async (userData) => {
    try {
      await axios.patch(`${API}/admin/users/${selectedUser.id}`, userData);
  ...
  const tabs = [
    ...
    { id: "dailyops", label: "Daily Ops", icon: <Zap size={18} /> },
  ...
  return (
    <div className="admin-page">
      ...
      <main className="admin-main">
        {activeTab === "dashboard" && (
          <div className="admin-dashboard">
            <div className="section-header">
              <h2>Morning glance</h2>
              <p>Mode: <strong>{stats.distribution_mode || "—"}</strong>
              {stats.as_of ? ` · as of ${new Date(stats.as_of).toLocaleTimeString()}` : ""}</p>
            </div>
            <div className="stats-grid">
              <div className="stat-card"><Users size={32} /><div className="stat-value">{stats.users?.total || 0}</div><div className="stat-label">Users (+{stats.users?.new_7d || 0} / 7d)</div></div>
              <div className="stat-card"><CreditCard size={32} /><div className="stat-value">{(stats.deposits_today?.btc_count || 0) + (stats.deposits_today?.manual_count || 0)}</div><div className="stat-label">Deposits today</div></div>
              <div className="stat-card highlight"><Wallet size={32} /><div className="stat-value">${((stats.deposits_today?.btc_gross_usd || 0) + (stats.deposits_today?.manual_gross_usd || 0)).toFixed(2)}</div><div className="stat-label">Gross today (fees ${Number(stats.deposits_today?.fees_usd || 0).toFixed(2)})</div></div>
            </div>
            <div className="stats-grid">
              <button type="button" className="stat-card queue-card" data-testid="dash-queue-distributor" onClick={() => setActiveTab("dailyops")}>
                <Zap size={32} /><div className="stat-value">{stats.distribution?.awaiting_send || 0}</div>
                <div className="stat-label">Awaiting send ({stats.distribution?.failed || 0} failed)</div>
              </button>
              <button type="button" className="stat-card queue-card" data-testid="dash-queue-payouts" onClick={() => setActiveTab("extensions")}>
                <ArrowUpFromLine size={32} /><div className="stat-value">{stats.queues?.payouts_hold || 0}</div>
                <div className="stat-label">Payouts on hold → compliance</div>
              </button>
              <button type="button" className="stat-card queue-card" data-testid="dash-queue-kyc" onClick={() => setActiveTab("extensions")}>
                <Shield size={32} /><div className="stat-value">{stats.queues?.kyc_pending || 0}</div>
                <div className="stat-label">KYC pending → compliance</div>
              </button>
              <button type="button" className="stat-card queue-card" data-testid="dash-queue-tickets" onClick={() => setActiveTab("dailyops")}>
                <MessageCircle size={32} /><div className="stat-value">{stats.queues?.open_alerts ?? "—"}</div>
                <div className="stat-label">Open alerts · tickets in Daily Ops</div>
              </button>
            </div>
          </div>
        )}
        ...
        {activeTab === "competition" && (
          <div className="admin-section">
            <Suspense fallback={<LoadingFallback />}><AdminCompetition /></Suspense>
          </div>
        )}
        ...
        {activeTab === "users" && (
          <div className="admin-section">
            ...
            <div className="data-table">
              <table>
                <thead><tr><th>Name</th><th>Email</th><th>Game credits</th><th>Tokens</th><th>Role</th><th>Actions</th></tr></thead>
                <tbody>
                  {users.map((u) => (
                    <tr key={u.id}>
                      <td>{u.name}</td>
                      <td>{u.email}</td>
                      <td>{Number(u.game_credits || 0).toLocaleString()}</td>
                      <td>{Number(u.sugar_tokens || 0).toLocaleString()}</td>
                      <td><span className={`badge ${u.role === "admin" ? "active" : "inactive"}`}>{u.role || "user"}</span></td>
                      <td>
                        <button className="btn-sm" onClick={() => { setSelectedUser(u); setShowUserModal(true); }}>Edit</button>
                        <button className="btn-sm primary" onClick={() => { setSelectedUser(u); setShowPaymentModal(true); }}>Adjust</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {activeTab === "transactions" && (
          <div className="admin-section">
            <div className="section-header">
              <h2>Money feed</h2>
              <p>Every rail, newest first — same feed as the player Ledger.</p>
            </div>
            <AdminTxFilter onChange={async (kind) => {
              try {
                const { data } = await axios.get(`${API}/admin/transactions`, { params: { kind, limit: 100 } });
                setTransactions(data.transactions || []);
              } catch { toast.error("Failed to filter feed"); }
            }} />
            <div className="data-table">
              <table>
                <thead><tr><th>Date</th><th>Type</th><th>User</th><th>Detail</th><th>Amount</th><th>Status</th></tr></thead>
                <tbody>
                  {transactions.map((tx) => {
                    const meta = TX_KIND_META[tx.kind] || { label: tx.kind };
                    return (
                      <tr key={`${tx.kind}-${tx.id}`}>
                        <td>{tx.created_at ? new Date(tx.created_at).toLocaleString() : "—"}</td>
                        <td>{meta.label}</td>
                        <td>{tx.user_email || "—"}</td>
                        <td>{txDetailText(tx) || "—"}</td>
                        <td>{txAmountText(tx)}</td>
                        <td><span className={`badge tx-status-${txStatusTone(tx.status)}`}>{tx.status ? String(tx.status).replace(/_/g, " ") : "—"}</span></td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        {activeTab === "dailyops" && (
          <div className="admin-section">
            <Suspense fallback={<LoadingFallback />}><AdminDailyOps /></Suspense>
          </div>
        )}
      ...
      {showPaymentModal && selectedUser && <AdjustBalanceModal user={selectedUser} onSave={handleAdjustBalance} onClose={() => { setShowPaymentModal(false); setSelectedUser(null); }} />}
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content user-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="form-section">
          ...
          <input value={gameUsername} onChange={(e) => setGameUsername(e.target.value)} placeholder="Username on the game backends" />
        </div>
        ...
        <button className="btn-primary" onClick={() => onSave({ game_accounts: gameAccounts, game_password: gamePassword, game_username: gameUsername, role })}>Save</button>
      </div>
    </div>
  );
};
...
const AdjustBalanceModal = ({ user, onSave, onClose }) => {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <button className="modal-close" onClick={onClose}>&times;</button>
        <h2>Adjust: {user.name}</h2>
        <p className="modal-subtitle">{user.email} · balances update immediately, every edit is audited</p>
        <form onSubmit={(e) => {
          e.preventDefault();
          onSave({ adjust_game_credits: Number(credits) || 0, adjust_sugar_tokens: Number(tokens) || 0, note });
        }}>
          <div className="form-group">
            <label>Game credits (+/-) · 100 = $1</label>
            <input type="number" value={credits} onChange={(e) => setCredits(e.target.value)} />
          </div>
          <div className="form-group">
            <label>Sugar tokens (+/-)</label>
            <input type="number" value={tokens} onChange={(e) => setTokens(e.target.value)} />
          </div>
          <div className="form-group">
            <label>Audit note (required for the trail)</label>
            <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="e.g. goodwill comp, event prize" required />
          </div>
...
const AdminTxFilter = ({ onChange }) => {
  const [kind, setKind] = useState("all");
  const kinds = [["all", "All"], ["purchase", "Purchases"], ["btc_deposit", "BTC in"],
    ["manual_deposit", "Cash in"], ["redemption", "Cash-outs"], ["giftcard", "Gift cards"],
    ["grant", "Grants"], ["adjustment", "Adjustments"]];
  return (
    <div className="ledger-filters" data-testid="admin-tx-filter">
      {kinds.map(([id, label]) => (
        <button key={id} type="button" className={`chip-btn ${kind === id ? "active" : ""}`}
          onClick={() => { setKind(id); onChange(id); }}>
          {label}
        </button>
      ))}
    </div>
