...
import {
  ...
  Clock, ChevronDown, Sparkles, Users, Lock, Trophy, BadgeCheck
} from 'lucide-react';
...
/* ---------------------------------------------------------------------------
 * timeAgo — "2h ago" style relative timestamps for the payout feed.
 * ------------------------------------------------------------------------ */
const timeAgo = (iso) => {
  if (!iso) return '';
...
const StatCounter = ({ end, label, prefix = '', suffix = '', decimals = 0 }) => {
  ...
  return (
    <div className="lp-stat" data-testid="stat-counter">
      <span className="lp-stat-num">{prefix}{shown}{suffix}</span>
...
/* ---------------------------------------------------------------------------
 * Live proof hooks — real numbers from /api/public/*. On ANY failure the
 * sections hide themselves: this page shows live data or nothing, never
 * placeholder figures.
 * ------------------------------------------------------------------------ */
...
const useRecentPayouts = () => {
  const [payouts, setPayouts] = useState([]);
  const [state, setState] = useState('loading'); // loading | ready | hidden
  useEffect(() => {
    let alive = true;
    const load = () => fetch(`${API}/public/payouts/recent?limit=12`)
      .then((r) => { if (!r.ok) throw new Error('payouts'); return r.json(); })
      .then((data) => { if (alive) { setPayouts(data.payouts || []); setState('ready'); } })
      .catch(() => { if (alive) setState('hidden'); });
    load();
    const poll = setInterval(load, 60000); // fresh ovations every minute
    return () => { alive = false; clearInterval(poll); };
  }, []);
  return { payouts, state };
};
...
const FAQ_ITEMS = [
  {
    ...
    a: 'Yes — and you don\u2019t have to take our word for it. The Standing Ovations feed on this page shows real completed payouts, latest first. Eligible winnings (gameplay wins plus winnings from free credits, minimum $50 / 5,000 credits) are paid in Bitcoin straight to your wallet or as gift cards. Redemptions of $500+ pass a manual review, then pay out. Every Bitcoin payout is an on-chain transaction you can verify yourself.',
  },
  {
    q: 'Who gives you the game credits?',
    a: 'We buy credits wholesale from a network of partner sweepstakes portals, then resell them to players. We are a retail-to-wholesale operator — we never manufacture or mint credits in-house, and we are not affiliated with any game manufacturer.',
  },
  {
    q: 'Why are purchased tokens not redeemable?',
    a: 'WAH-LAH uses a dual-currency model. Tokens you purchase are for play only and cannot be cashed out. Only credits earned through our free daily AMOE (alternative method of entry) and credits you win through gameplay are redeemable.',
  },
  {
    q: 'Are there fees on deposits or payouts?',
    a: 'A resale margin is built into each funding method: Bitcoin and crypto ~10%, Cash App ~12%, gift cards ~5%. Redemptions of eligible credits are paid out free of those purchase fees.',
  },
  {
    q: 'Who owns the brands in The Bill?',
    a: 'Fire Kirin, Juwa, Orion Stars and the other platforms in our lobby are operated by independent game vendors. WAH-LAH is not the manufacturer of any of them — we are the venue that connects you to all seven behind one account.',
  },
];
...
const LandingPage = () => {
  ...
  const { stats, state: statsState } = usePublicStats();
  const { payouts, state: payoutsState } = useRecentPayouts();
  const showTicker = payoutsState === 'ready' && payouts.length > 0;
  ...
  return (
    <div className="lp" data-testid="landing-page">
      ...
      <nav className={`lp-nav ${scrolled ? 'lp-nav-scrolled' : ''}`} data-testid="landing-nav">
        <div className="lp-nav-inner">
          ...
          <div className="lp-nav-links">
            ...
            {payoutsState !== 'hidden' && (
              <a href="#ovations" className="lp-nav-link" data-testid="nav-ovations">Winners</a>
            )}
      ...
      {/* ------------------ OVATION TICKER (live, hides when empty) ------------------ */}
      {showTicker && (
        <div className="lp-ticker" data-testid="ovation-ticker" aria-label="Recent payouts">
          <div className="lp-ticker-track">
            {[...payouts, ...payouts].map((p, i) => (
              <span key={i} className="lp-ticker-item">
                <Trophy size={13} />
                {p.name} cashed out ${Number(p.amount_usd).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} in {p.method}
                <em>{timeAgo(p.paid_at)}</em>
              </span>
            ))}
          </div>
        </div>
      ...
      {/* ------------------ STATS (live, hides if unreachable) ------------------ */}
      {statsState === 'loading' && (
        <section className="lp-stats" aria-label="Loading live stats" data-testid="stats-loading">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="lp-stat"><span className="lp-shimmer" /></div>
          ))}
        </section>
      )}
      {statsState === 'ready' && stats && (
        <section className="lp-stats" id="stats" data-testid="stats-section">
          <StatCounter end={stats.players} label="Active Players" />
          <StatCounter end={stats.paid_out_usd} label="Paid Out to Players" prefix="$" decimals={2} />
          <StatCounter end={stats.platforms} label="Game Platforms" />
          <StatCounter end={stats.payouts_count} label="Payouts Completed" />
        </section>
      )}
      ...
      {/* ------------------ STANDING OVATIONS (live payout proof) ------------------ */}
      {payoutsState !== 'hidden' && (
        <section className="lp-ovations" id="ovations" data-testid="ovations-section">
          <div className="lp-section-head">
            <span className="lp-section-tag">Encore, Encore</span>
            <h2 className="lp-section-title">Standing Ovations</h2>
            <p className="lp-section-desc">
              Real payouts to real players, latest first. Proof beats promises.
            </p>
          </div>
          {payoutsState === 'loading' && (
            <div className="lp-ovations-grid" aria-label="Loading recent payouts">
              {[0, 1, 2].map((i) => (
                <div key={i} className="lp-ovation-card"><span className="lp-shimmer lp-shimmer-block" /></div>
              ))}
            </div>
          )}
          {payoutsState === 'ready' && payouts.length > 0 && (
            <div className="lp-ovations-grid">
              {payouts.slice(0, 6).map((p, idx) => (
                <div key={idx} className="lp-ovation-card" data-testid={`ovation-card-${idx}`}>
                  <div className="lp-ovation-top">
                    <span className="lp-ovation-avatar" aria-hidden>
                      {p.name ? p.name.charAt(0).toUpperCase() : '?'}
                    </span>
                    <div className="lp-ovation-who">
                      <strong>{p.name}</strong>
                      <span>{timeAgo(p.paid_at)}</span>
                    </div>
                    <span className="lp-ovation-badge">
                      {p.method === 'BTC' ? <Bitcoin size={13} /> : <Gift size={13} />}
                      {p.method}
                    </span>
                  </div>
                  <div className="lp-ovation-amount">
                    ${Number(p.amount_usd).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </div>
                  <div className="lp-ovation-paid">
                    <BadgeCheck size={14} /> Paid out
                  </div>
                </div>
              ))}
            </div>
          ...
          {payoutsState === 'ready' && payouts.length === 0 && (
            <div className="lp-ovations-empty" data-testid="ovations-empty">
              <Trophy size={28} />
              <h3>The stage is set. The first ovation goes to&hellip;</h3>
              <p>No payouts yet — this feed only ever shows real completed payouts, so it starts honest and empty. Be the first name in lights.</p>
              <Link to="/register" className="lp-btn-primary" data-testid="ovations-register-btn">
                Claim 100 Free Credits <ArrowRight size={18} />
              </Link>
            </div>
          )}
        </section>
      ...
      {/* ------------------ FAQ ------------------ */}
      <section className="lp-faq" id="faq" data-testid="faq-section">
        <div className="lp-section-head">
          <span className="lp-section-tag">Straight Answers</span>
          <h2 className="lp-section-title">Frequently Asked Questions</h2>
          <p className="lp-section-desc">No sleight of hand here — how credits work, where they come from, and what the fees really are.</p>
        </div>
        <div className="lp-faq-list">
          {FAQ_ITEMS.map((item, idx) => (
            <div key={idx} className={`lp-faq-item ${faqOpen === idx ? 'lp-faq-open' : ''}`} data-testid={`faq-item-${idx}`}>
              <button
                type="button"
                className="lp-faq-q"
