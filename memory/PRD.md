...
- **Cashtag wired**: `CARD_PAYMENT_TAG=REDACTED_CASHTAG` in env. Verified `GET /api/payment/card-info` returns `{"tag":"REDACTED_CASHTAG","instructions":"Send payment via Cash App or Chime ..."}`. Same tag covers Chime per user instruction.
