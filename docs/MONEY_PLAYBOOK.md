...
These are the three places WAH-LAH makes you money. All fees are **disclosed** to the player on the relevant screen so they can never claim they didn't know.

---

## Daily Ops — Cash App / Chime Deposit Reconcile

1. Player sends **$25** to `$CARD_PAYMENT_TAG` (set in env; never commit the real tag) with their game tag in the note
...
> **Status**: automated `send_btc` exists in `services/btc_payout.py` (funding wallet via `BTC_PAYOUT_XPRV`, admin approve-then-send). Verify `BTC_PAYOUT_XPRV` is declared on Render before relying on the automated path — otherwise keep sending BTC manually from the distributed wallet.

---

## Pool Health — Seats, Balances, Credit Rebalancer (LIVE)

The distributor proxy pool runs three automated loops in the web process (all non-fatal on start error):

- **On-duty watchdog** — `/api/onduty/status` probes mongo/api/frontend/auth + stale-deposit scan; emails the operator on degradation/recovery.
- **Deposit reconciler** — periodic `/api/deposit/sync` sweep auto-settles webhook-missed confirmed BTC deposits.
- **Pool balance resync** (new) — every `POOL_RESYNC_INTERVAL_MIN` (24h default) refreshes each active/cooldown seat's real balance and runs a **dry-run** rebalance on the fresh picture.

Per-seat semantics (audited to `pool_resyncs`, admin `GET /api/ext/pool/admin/resyncs`):

| Bridge result | Effect |
|---|---|
| success | `balance_cached` refreshed, `consecutive_failures=0`, reactivated, cooldown cleared |
| failure | `mark_failed` (cooldown/lock progression), cached balance untouched |
| **unsupported** (hub has no balance endpoint) | NOT a failure — cached balance stays "unknown", floor rule can't mis-disable |

Admin endpoints:

```bash
POST /api/ext/pool/admin/resync            # run a balance refresh pass now
GET  /api/ext/pool/admin/resyncs?limit=10  # recent audit rows
POST /api/ext/pool/admin/health/rebalance?dry_run=true   # default DRY (safe)
POST /api/ext/pool/admin/health/rebalance?dry_run=false  # REAL sweeps — only after
                                                         # live balance data exists
```

Gates in render.yaml / `.env.example`:
- `POOL_PULL_ENABLED=true` — pull (reclaim player → seat credits) fires only when a hub **and** proxy both declare `pull_supported` (none do yet → clean wholesale fallback, never blocks/fails a redemption).
- `POOL_REBALANCE_ENABLED=true` — sweep/capacity rules enabled; **real** sweeps still require explicit `dry_run=false`.
- `POOL_RESYNC_ENABLED=true`, `POOL_RESYNC_INTERVAL_MIN=1440`, `POOL_RESYNC_RUN_REBALANCE=true` (post-pass rebalance is dry-run).

**Prerequisite for real sweeps**: add `api_paths.balance` (sugar_sweeps, bitbetwin) / verify `selectors.balance` (Playwright hubs) so `balance_cached` becomes ground truth. Until then resync reports all hubs unsupported and floor/sweep safely no-op on unknown balances.
...
2. Player deposits → instead of auto-routing through a third-party proxy, the
   system queues a task: **"Send $5.00 to 'player1' on Fire Kirin"**
   (task shows 500 internal Game Credits = $5.00 platform dollars — games
   display dollars, so you type the dollar figure, not the credit figure).
3. Pull the queue, open that game's backend, type the username + the dollars,
   hit send:

   ```bash
   GET /api/ext/distributor/queue
   # → every pending task: platform, recipient username, platform_amount ($)
   ```
...
on deposit (`cashtag` 12%) with zero cost of goods. Flip back
...
At 100 active players, average $40 deposit/month, ~70% Cash App:
