...
1. Go to https://withpersona.com → **Sign up** with `REDACTED_EMAIL`
...
## 5. Push the keys to Render
Once you have all 4, add them as environment variables in the **Render** dashboard (Service `real-wah-lah-com` → **Environment**):
```text
...
```
Saving the env vars restarts the service automatically — no redeploy needed.

That's it. The code at `/app/backend/services/compliance/persona.py` auto-detects the env vars and flips `PERSONA_ENABLED=True` on next boot. Redemptions ≥$500 will trigger Basic, ≥$5000 will trigger Enhanced.
...
4. Update the Render env vars with `PERSONA_ENVIRONMENT=production` + the new prod values (Save = restart)
