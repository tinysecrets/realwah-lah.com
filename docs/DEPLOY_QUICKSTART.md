...
flyctl secrets set \
  ...
  CARD_PAYMENT_TAG='REDACTED_CASHTAG' \
...
```bash
flyctl secrets set \
  SUGAR_SWEEPS_USERNAME='REDACTED_EMAIL' \
...
| Player payments (Cash App / Chime) | Tag `REDACTED_CASHTAG` — manual reconcile in Admin → Transactions |
...
4. **Cash App / Chime deposit** → player sends to `REDACTED_CASHTAG` with their game tag in the note → admin manually credits via Admin → Transactions → "Mark Paid."
