...
from the repository root .env. Never stores the chosen password in Git.
"""
...
email = input("Admin email [admin@wah-lah.com]: ").strip().lower() or "admin@wah-lah.com"
...
try:
    ...
    result = users.update_one({"email": email}, update, upsert=True)
    user = users.find_one({"email": email}, {"password_hash": 0})

    action = "updated" if existing else "created"
    print(f"\nSUCCESS: admin account {action}")
    print(f"Email: {user['email']}")
    print(f"Role:  {user.get('role')}")
    print(f"DB:    {db_name}")
    print("Password hash stored with bcrypt; the plaintext password was not saved.")
finally:
    client.close()
